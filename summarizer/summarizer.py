...
⚠️ 核心要求：
- 总共输出 18-22 条新闻
- 至少 5 条必须与AI/人工智能相关（标题前加 [AI相关] 标记）
- 严格去重：同一新闻只保留一条
- **每条开头标注日期**，格式为 [MM-DD]

每条新闻前面标注了 [#N] 编号（N为数字），这是该新闻的唯一编号。
你必须在每条新闻末尾标注来源编号，格式：[来源：XXX #N]。
...
### AI与科技（至少5条，放最前面）
- [MM-DD] [AI相关] [标题]：一句话摘要 [来源：XXX #N]

### 要闻精选（8-10条）
- [MM-DD] [标题]：一句话摘要 [来源：XXX #N]
...
要求：
1. 共输出18-22条，每条控制在50字以内，末尾标注编号
2. AI与科技栏目放最上方，至少5条带 [AI相关] 标记
3. 每条开头用 [MM-DD] 标注日期
4. **选择性高亮**：仅将你认为最有价值的1-3条用 **加粗** 标出（不要全部标红）
5. **重点关注**：公益、正能量、民生改善等相关内容优先收录
6. 浙江/杭州区域栏目重点聚焦杭州本地的政策、经济、科技、民生等新闻
7. 按重要程度排序，客观陈述不加评论
8. 严格去重：同一新闻无论来自几个源，只保留编号最小的那条
"""
...
每条新闻前面标注了 [#N] 编号（N为数字），这是该新闻的唯一编号。
**你必须在每条新闻的末尾标注来源时附上对应的编号**，格式为 [来源：XXX #N]。

⚠️ 编号准确性要求：
- 来源编号必须与新闻前的 [#N] 严格对应，不能错位
- 例如：[#5] TechCrunch 的文章 → 必须用 [来源：TechCrunch #5]
- 绝对禁止出现来源与编号不匹配的情况
- 如果不确定某个新闻的编号，请用 [来源：XXX]（不带编号）宁缺毋滥

输出格式（每条末尾加 [来源：XXX #N]，每条开头加日期 [MM-DD]）：

## 🎯 AIGC行业日报

### 🔥 今日焦点
- [MM-DD] [标题]：核心内容 + 行业影响分析 [来源：XXX #N]

### 🏢 大公司动态
- [MM-DD] [公司名]：[动态简述 + 战略意义] [来源：XXX #N]

### 🚀 产品技术更新
（重点：AI视频生成、AI影视工具、AI广告平台、数字人、AI配音等）
- [MM-DD] [产品/技术名]：[更新内容 + 对AI影视/广告的影响] [来源：XXX #N]

### 📊 市场与投融资
- [MM-DD] [事件]：[金额/规模 + 趋势判断] [来源：XXX #N]

### 🏆 AIGC比赛资讯（如有相关新闻则输出此栏目，无则跳过）
- [MM-DD] [比赛名称]：[主办方、赛道、奖金、截止时间等关键信息] [来源：XXX #N]

### 💡 对AI影视传媒从业者的启示
从AI漫剧短剧、AI广告、AI宣传片的角度，总结3-5条 actionable 洞察（每条末尾不加来源）

要求：
1. 每条控制在80字以内，末尾标注 [来源：XXX #N]
2. 每条开头用 [MM-DD] 标注日期
3. **重点聚焦**：AI视频生成、AI短剧、AI影视制作、AI广告投放、AI宣传片、数字人、AI配音等领域
4. 过滤掉与AI影视传媒无关的纯硬件/底层技术新闻
5. 严格去重，相同内容只保留一条，优先保留编号更小的版本
6. 如有比赛信息，在"🏆 AIGC比赛资讯"栏目中汇总
7. 提供可执行的商业洞察
"""
...
def _format_articles_for_prompt(articles: List[Dict], max_count: int = 30, start: int = 1) -> str:
    ...
    for offset, art in enumerate(articles[:max_count]):
        ...
        # 提取日期
        pub_date = ""
        for date_field in ["published", "collected_at", "report_date"]:
            raw = art.get(date_field, "")
            if raw:
                try:
                    pub_date = datetime.fromisoformat(raw.replace("Z", "+00:00")).strftime("%m-%d")
                    break
                except Exception:
                    try:
                        pub_date = datetime.strptime(str(raw)[:10], "%Y-%m-%d").strftime("%m-%d")
                        break
                    except Exception:
                        continue
        date_tag = f" [{pub_date}]" if pub_date else ""
        lines.append(f"[#{i}]{date_tag} [{source}] {title}")
        if link:
...
def summarize_political(news_list: List[Dict], client: Optional[ClaudeClient] = None) -> str:
    ...
    try:
        result = client.chat(POLITICAL_SUMMARY_SYSTEM, [{"role": "user", "content": prompt}])
        if not result or not result.strip():
            logger.warning("AI返回空内容，使用备用总结")
